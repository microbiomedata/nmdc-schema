...
### Valid examples: no derived files in PRs
Never commit generated/derived files like `nmdc_schema/nmdc.py` or
`nmdc_schema/nmdc_materialized_patterns.yaml` in schema PRs. These
are regenerated by `make all`. If they show up in your diff, restore
them: `git checkout origin/main -- nmdc_schema/nmdc.py nmdc_schema/nmdc_materialized_patterns.yaml`
