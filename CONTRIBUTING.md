...
- Put example values in the `examples` metaslot, not in prose
    ...
    - When an example needs a human-readable gloss, keep the two together with the LinkML `Example` fields: `value:` plus `description:` (for instance `value: ENVO:00002297` with `description: desert ecosystem`). Scalar examples, including measurements written as a number plus unit (`20 Cel`), go in `value:` (quote them). Reserve `object:` for a structured-instance example (a `QuantityValue`, `TextValue`, or similar), not for plain scalars.
