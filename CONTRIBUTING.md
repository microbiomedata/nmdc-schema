...
- Put example values in the `examples` metaslot, not in prose
    ...
    - Indicators that a text annotation is holding a misplaced example (the value should move to `examples`):
        ...
        - The value could stand on its own as a filler for the slot: a CURIE (`ENVO:00002297`), a literal label (`total phosphorus`), or an enum value. (A unit-bearing measurement is not a string filler; see the units note below.)
    ...
    - When an example needs a human-readable gloss, keep the two together with the LinkML `Example` fields: `value:` plus `description:` (for instance `value: ENVO:00002297` with `description: desert ecosystem`). Scalar examples (a CURIE, a label, an enum value) go in `value:` (quote them). Reserve `object:` for a structured-instance example such as a `QuantityValue` or `TextValue`. Do not put a measurement and its unit in a scalar string (`20 Cel`): NMDC couples the unit to the slot (a `QuantityValue` range, or a units annotation) and records the magnitude as a bare number, so such an example is a structured value, not a `value:` string.
