...
gendoc: $(DOCDIR) prefixmaps
	...
	#
	# Note: We use `[ -f path ]` to check whether the file exists, and ` || true` so that
	#       the overall command returns exit code 0 either way. The files being copied here
	#       are generated by the `all` target, which may not have been run yet.
	#
