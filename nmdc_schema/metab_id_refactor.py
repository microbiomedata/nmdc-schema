...
def update_data_products(
    nom_activities_doc,
    new_raw_file_id: str,
    new_data_product_id: str,
    omics_prcessing_id: str,
    ...
    raw_object_docs = [
        raw_objectdata_doc
        for raw_objectdata_doc in data_object_set.find(get_raw_file_data_object)
    ]
    ...
    if raw_object_docs:
        raw_object_update = {
            "$set": {
                "id": new_raw_file_id,
                "alternative_identifier": [omics_prcessing_id],
            }
        }
        ...
        new_raw_data_object = get_raw_data_object(
            raw_file_path,
    ...
    for data_product_objectdata_doc in data_object_set.find(
        get_data_product_data_object
    ):
        data_product_object_update = {"$set": {"id": new_data_product_id}}

        data_object_set.update_one(
            data_product_objectdata_doc, data_product_object_update
        )


def update_omics_processing(
    nom_new_id, new_data_product_id, new_raw_file_id, raw_file_path=None
):
    ...
    for nom_activities_doc in nom_activities_set.find(get_old_activities):
        ...
        for omics_processing_doc in omics_processing_set.find(
            get_parent_omics_processing
        ):
            omics_processing_update = {"$set": {"has_output": [new_raw_file_id]}}

            omics_processing_set.update_one(
                omics_processing_doc, omics_processing_update
            )
            ...
            nom_activity_update = {
                "$set": {
                    "id": nom_new_id,
                    "has_output": [new_data_product_id],
                    "has_input": [new_raw_file_id],
                    "was_informed_by": [new_omics_processing_id],
                }
            }
...
def mint_nmdc_id(type: NMDC_Types, how_many: int = 1) -> List[str]:
    ...
    token = oauth.fetch_token(
        token_url="https://api.microbiomedata.org/token",
...
def get_raw_data_object(
    file_path: Path, was_generated_by: str, data_object_type: str, description: str
) -> nmdc.DataObject:
