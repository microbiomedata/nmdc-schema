...
def get_migrator_classes() -> List[Type[MigratorBase]]:
    ...
    Part 4 renames `PersonValue`
    to `Person`, drops `has_raw_value`, and renames `applies_to_person` to
    `applies_to_agent`.
    Part 5 removes the deprecated `collection_date_inc` slot from Biosample; it
    is independent of parts 1 through 4.
