...
def get_collection_names_with_qv_slots_from_schema() -> List[str]:
    ...
    for slot_name in schema_view.class_slots(DATABASE_CLASS_NAME):
        ...
        if slot_definition.multivalued and slot_definition.inlined_as_list:
            ...
            if _collection_could_contain_quantity_values(
                schema_view, slot_definition.range
            ):
...
class Migrator(MigratorBase):
    ...
    QUANTITY_VALUE_UNITS = {
        "nmdc:PortionOfSubstance": {"final_concentration": "%", "volume": "mL"},
        "nmdc:Biosample": {
            "humidity": "%",
            "tot_carb": "%",
            "tot_nitro_content": "%",
        ...
        "nmdc:ChemicalConversionProcess": {"temperature": "Cel", "duration": "h"},
        "nmdc:ChromatographyConfiguration": {"temperature": "Cel"},
        "nmdc:Extraction": {"input_mass": "g"},
        "nmdc:SubSamplingProcess": {"mass": "g"},
        "nmdc:MobilePhaseSegment": {"duration": "min"},
    }
        ...
        if isinstance(self.adapter, MongoAdapter):
            ...
            try:
                ...
                if commit_changes:
                    ...
                    self.logger.info(
                        "Migration validation passed. Transaction rolled back (no changes were committed)"
                    )
                ...
                if commit_changes:
                    self.logger.info(
                        "Migration validation failed. Transaction automatically rolled back (no changes were saved, despite commit request)"
                    )
                else:
                    self.logger.info(
                        "Migration validation failed. Transaction rolled back (no changes were committed)"
                    )
            ...
            try:
                for collection_name in real_collection_names:
                    self.adapter.process_each_document(
                        collection_name, [self.ensure_quantity_value_has_unit]
                    )
                ...
                if not commit_changes:
                    self.logger.info(
                        "Migration validation passed. Note: Non-MongoDB adapter doesn't support rollback - changes are applied immediately"
                    )
                else:
    ...
    def _traverse_and_fix_quantity_values(
        self, obj: Any, document_root: dict, path: str = ""
    ) -> None:
        """
        ...
        if isinstance(obj, dict):
            ...
            if obj.get("type") == "nmdc:QuantityValue":
                ...
                for key, value in obj.items():
                    ...
                    self._traverse_and_fix_quantity_values(
                        value, document_root, new_path
                    )
    ...
    def _fix_quantity_value_unit(
        self, quantity_value: dict, document_root: dict, path: str
    ) -> None:
        r"""
        ...
        if "has_unit" not in quantity_value or quantity_value["has_unit"] is None:
            ...
            most_specific_class = get_most_specific_class_for_reporting(
                self._schema_view, document_root, path
            )

            # Check for special cases where we can extract unit from raw_value
            unit = self._handle_one_off_unit_cases(
                quantity_value, most_specific_class, path, None
            )
            ...
            if unit:
                ...
                self.reporter.track_missing_value(
                    root_collection_class, clean_schema_path
                )
        else:
            ...
            if current_unit in self._unit_alias_map:
                ...
                handled_unit = self._handle_one_off_unit_cases(
                    quantity_value, most_specific_class, path, current_unit
                )
                if handled_unit:
                    ...
                    self.reporter.track_unmapped_value(
                        root_collection_class, clean_schema_path, current_unit
                    )
    ...
    def _infer_unit_from_context(self, full_document: dict, path: str) -> Optional[str]:
        ...
        root_class = (
            doc_type.replace("nmdc:", "") if doc_type.startswith("nmdc:") else doc_type
        )

        # Use schema to resolve the class context for this field
        target_class = resolve_class_from_schema_path(
            self._schema_view, root_class, slot_path
        )
        if target_class:
            return self._get_unit_for_class_slot(
                f"nmdc:{target_class}", field_name, None
            )

        # Fallback to document type if schema resolution fails
        return self._get_unit_for_class_slot(doc_type, field_name, None)

    def _add_unit_to_quantity_value(
        self,
        quantity_value: dict,
        class_uri: str,
        slot_name: str,
        ...
        if "has_unit" not in quantity_value or quantity_value["has_unit"] is None:
            ...
            if current_unit in self._unit_alias_map:
                ...
                if current_unit != canonical_unit:
                    ...
                    self.reporter.track_operation(
                        "units_normalized", f"{current_unit} → {canonical_unit}", 1
                    )
            else:
                # Current unit is not in our alias map
                # Check for special one-off cases first
                if self._handle_one_off_unit_cases(
                    quantity_value, class_uri, slot_name, current_unit
                ):
                    # One-off case was handled, continue to next
                    pass
                elif (
                    current_unit in self._unit_alias_map
                    and self._unit_alias_map[current_unit] == current_unit
                ):
                    ...
                    self._get_unit_for_class_slot(
                        lookup_class_uri, slot_name, current_unit
                    )
                    # If no mapping found, just report it but don't change the value

    def _handle_one_off_unit_cases(
        self,
        quantity_value: dict,
        class_uri: str,
        path: str,
        ...
        # Handle existing units that need conversion
            ...
            if (
                ...
                and slot_name == "nitrate"
                and current_unit == "detection"
            ):
    ...
    def _get_unit_for_class_slot(
        self, class_uri: str, slot_name: str, current_unit_value: str = None
    ) -> Optional[str]:
        r"""
        ...
        if (
            current_unit_value is not None
            and current_unit_value not in self._unit_alias_map
        ):
            ...
            self.reporter.track_value_set(
                "unmapped_values", slot_key, current_unit_value
            )

        # Return None instead of a fallback unit - just report and don't update
        return None

    def _get_unit_from_inheritance_chain(
        self, class_uri: str, slot_name: str
    ) -> Optional[str]:
        r"""
