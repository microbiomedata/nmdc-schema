"""Prototype runner for LinkML-oriented embeddings alignment comparisons."""
...
@click.option(
    "--semantic-results",
    required=True,
    help="TSV of semantic candidates, e.g. OLS4 embeddings results from PR #2909",
)
@click.option(
    "--output",
    "-o",
    default="local/linkml_embeddings_alignment_enriched.tsv",
    show_default=True,
    help="Output TSV path for enriched candidate rows",
)
@click.option(
    "--summary-json",
    default="local/linkml_embeddings_alignment_summary.json",
    show_default=True,
...
def cli(
    ...
    """Enrich semantic hits with schema context and local lexical diagnostics."""
