...
# Top-level trees that are generated, vendored, or themselves the thing being audited.
# Matched only as the first path component: `src/docs/` and `units/docs/` are authored
# content and must stay searchable, even though the root `docs/` tree is generated.
ROOT_EXCLUDED_DIRS = frozenset(
    {
        ASSETS_DIR,
...
# Vendor/build noise that is excluded no matter how deep it is nested.
...
# Basenames that are not distinctive enough to trust the bare-filename fallback for:
# more than one tracked asset uses it, and/or the same name shows up incidentally
# elsewhere for something unrelated (a Makefile target's own output path, this
# repository's own top-level README.md). For these, only a full-path match counts.
GENERIC_BASENAMES = frozenset({"README.md", ".gitkeep"})
...
def find_asset_findings(repo_root):
    ...
    for asset in assets:
        ...
        referenced_by = sorted(
            ...
            if asset in text or (trust_basename and basename in text)
        )
