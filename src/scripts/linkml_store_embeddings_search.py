...
@click.option(
    ...
    default="OBI",
    show_default=True,
    help="Ontology prefix to harvest locally via oaklib and search with linkml-store",
)
@click.option(
    "--adapter-handle",
...
def cli(
    ...
    if restrict_to_baseline_source_hits:
        eligible_subject_ids = {
            ...
            if (row.get("object_source") or "").upper() == ontology_prefix.upper()
        }
    ...
    derived_cache_collection = cache_collection or f"{ontology_prefix.lower()}_{embedding_model_name.replace('-', '_')}"
    ...
    summary = {
        ...
        "overlap_vs_ols4": summarize_backend_overlap(
            reference_rows=baseline_rows,
            candidate_rows=rows,
            object_source=ontology_prefix,
            top_k=top_k,
        ),
    }
