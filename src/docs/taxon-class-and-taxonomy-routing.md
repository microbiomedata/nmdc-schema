...
| GTDB taxonomy | Same `Taxon` class, add `gtdb` to `id_prefixes`, load into `ontology_class_set` | Future — needs a GTDB-to-OntologyClass loader since GTDB ships as TSV, not OWL |
| LPSN / SeqCode nomenclature | Same pattern — CURIE-identified Taxon instances | Future, when needed |
