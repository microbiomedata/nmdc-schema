...
The central identifier minting service is an endpoint part of the NMDC runtime API. To use the minting service to generate identifiers for your _entities_, use this [endpoint](https://api.dev.microbiomedata.org/docs#/identifiers/mint_ids_ids_mint_post).
...
5. `.version`: This and subsequent components of the identifier come into picture when we are generating ids for metaproteomic analysis samples. The *version* portion of the contig ID is used to indicate the version of the workflow that was used to perform the analysis on the sample. The delimiter used to separate the *version* from the blade and everything before it, is a dot `.` and the version is also an alphanumeric string of length one to six characters.

6. `_locus`: This component is used to indicate the genetic coordinates, i.e., the segment of the genomic sample that is responsible for producing the protein. The intra component delimiter that is used to separate coordinates of the locus is underscore `_`. The locus is also an alphanumeric string composed of one or more characters which can include upper and lowercase letters, numbers, and special characters underscore `_`, dot `.` and hyphen `-`.
...
* The above identifier structure applies to ids minted by the central minting service, and offline site id minting services.
